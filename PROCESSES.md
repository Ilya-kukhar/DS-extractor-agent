...
| Изменение в Pack | Cross-Repo Sync | Триггер (будущее) |
| По расписанию / запросу | Knowledge Audit | Расписание / пользователь (будущее) |
...
## Процесс 4: Cross-Repo Sync (будущее)

> Проверка: изменение в Pack → нужно ли обновить downstream?

### Триггер

Git hook или ручной запуск после изменения Pack-файла.

### ВДВ

**Входы:** Изменённые файлы Pack'а + список downstream-репо.

**Действия:**
1. Определить, какие downstream-репо зависят от изменённых сущностей
2. Проверить, нужны ли обновления (CLAUDE.md, промпты, конфиг)
3. Сформировать список предложений

**Выходы:** Список предложений обновления downstream-репо.

**Статус:** Не реализован. Приоритет: после MVP (процессы 1-3).

---

## Процесс 5: Knowledge Audit (будущее)

> Периодическая ревизия всех Pack'ов.

### Триггер

По расписанию (ежемесячно) или по запросу.

### ВДВ

**Входы:** Все Pack-репозитории.

**Действия:**
1. Обход всех Pack'ов
2. Проверка: полнота (есть ли пробелы), устаревшие записи, битые ссылки, нарушения SPF
3. Формирование аудиторского отчёта

**Выходы:** Аудиторский отчёт с рекомендациями.

**Статус:** Не реализован. Приоритет: после Cross-Repo Sync.
