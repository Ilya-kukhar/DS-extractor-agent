...
- Да → продолжай
- Нет → скажи: «Это governance-контент (привязан к конкретному проекту/задаче). Не подлежит экстракции. Записать в `DS-my-strategy/inbox/`?»
...
| Созидатель, развитие | PACK-personal | `PP` |
| Экосистема, клуб | PACK-ecosystem | `EP` |
...
4. **Привяжи к родительскому понятию SPF.** Прочитай `SPF/ontology.md` секция 2 — найди подходящее U.* понятие. Например: «цифровая платформа» → U.System. Укажи «FPF-понятие» или «Родительское понятие (SPF)».
...
3. Предложи обновление downstream ontology.md (добавить понятие в соответствующую секцию).

### Шаг 6: Проверка противоречий и онтологии

1. **Прочитай** существующие сущности в целевой директории Pack'а.
2. **Прочитай** `01B-distinctions.md` целевого Pack'а.
3. **Прочитай** `ontology.md` целевого Pack'а (SPF.SPEC.002) — согласованы ли типы и термины.
4. **Прочитай** CLAUDE.md целевого репо.
5. **Прочитай** `ontology.md` downstream-репо (если сессия в downstream).
6. **Оцени:**
...
| **Расширяет онтологию** | Вводит новый тип/термин | Предложить обновление `ontology.md` |
| **Противоречит онтологии** | Тип/термин используется иначе | Показать расхождение, предложить варианты |
| **Доменное в downstream** | Понятие из downstream полезно для домена | Предложить добавить в Pack + ссылка в downstream |
| **Реализационное в downstream** | Специфично для этого downstream | Добавить в downstream ontology.md с привязкой к Pack |
